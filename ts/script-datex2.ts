...
async function loadContent(requestType) {
    ...
    const response = await fetch(`${TRAFFIC_MESSAGES_URL}?situationType=${requestType}`);
...
function processResponse(resp, requestType) {
    if (resp) {
        ...
        $("#date_" + requestType).text(
            "Updated: " + toLocalDate(resp.dataUpdatedTime) + " / " + resp.features.length + " pcs"
        );
