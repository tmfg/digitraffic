...
export function doSearch() {
  function displaySearchResults(results, store) {
    ...
    if (results.length) { // Are there any results?
      document.getElementById("search-results-amount").innerText =
        results.length;
      ...
      for (var i = 0; i < results.length; i++) { // Iterate over the results
        ...
        //Title
        appendString +=
          '<h3 class="h3 posts-in-category__post-heading"><a href="';
        if (item.category === "Kehityssuunnitelma") {
          appendString += window.baseurl + itemlang +
            "/kehityssuunnitelma?traffictype=" +
            item.traffictypes[0].toLowerCase() + "&status=" +
            item.status.toLowerCase();
        } else {
          appendString += window.baseurl + item.url;
        }
        appendString += '" class="posts-in-category__post-heading-link">' +
          item.title + "</a></h3>";
        ...
        if (item.category === "Page") {
          ...
          if (item.traffictypes && item.traffictypes.length > 0) {
            ...
            item.traffictypes.forEach((traffictype) => {
              appendString +=
                '<li class="date-type-tags__type date-type-tags__type--no-preceding-data"><a class="link" href="' +
                window.baseurl + item.url + '">' + traffictype + "</a></li>";
            });
            appendString += "</ul>";
          }
        } else {
          //Date
          if (
            item.category !== "Sovellukset" &&
            item.category !== "Kehityssuunnitelma"
          ) {
            appendString +=
              '<span class="date-type-tags__date"><i class="material-icons md-md date-type-tags__date-icon">';
            item.category === "Tapahtumat"
              ? appendString += "event"
              : appendString += "create";
          ...
          if (item.issuenumber && item.issuenumber.length > 0) {
            appendString += '<span class="date-type-tags__id">' +
              item.issuenumber + "</span>";
          }
          //Category
          let category = item.category.replace(/^[a-z][a-z],\s/g, ""); // Strip language from category, eg. "en, News" -> "News"
          appendString +=
            '<span class="date-type-tags__category"><a class="link" href="' +
            window.baseurl + itemlang + "/" + category.toLowerCase() + '">' +
            category + "</a></span>";
          //Traffictypes
          appendString += '<ul class="date-type-tags__type-list">';
          item.traffictypes.forEach((traffictype) => {
            appendString +=
              '<li class="date-type-tags__type"><a class="link" href="' +
              window.baseurl + itemlang + "/" + category.toLowerCase() +
              "?traffictype=" + traffictype.toLowerCase() + '">' + traffictype +
              "</a></li>";
          });
          appendString += "</ul>";
          //Tags
          if (item.tags && item.tags.length > 0) {
            ...
            item.tags.forEach((tag) => {
              appendString +=
                '<li class="date-type-tags__tag"><a class="link" href="' +
                window.baseurl + itemlang + "/" + category.toLowerCase() +
                "?tag=" + tag.toLowerCase() + '">' + tag + "</a></li>";
            });
            appendString += "</ul>";
          }
          //Environments
          if (item.environments && item.environments.length > 0) {
            ...
            item.environments.forEach((env) => {
              appendString +=
                '<li class="date-type-tags__tag"><a class="link" href="' +
                window.baseurl + itemlang + "/" + category.toLowerCase() +
                "?environment=" + env.toLowerCase() + '">' + env + "</a></li>";
            });
            appendString += "</ul>";
          }
          //Status
          if (item.status && item.status.length > 0) {
            appendString +=
              '<span class="date-type-tags__status"><a class="link" href="' +
              window.baseurl + itemlang + "/" + category.toLowerCase() +
        ...
        appendString += '<div class="posts-in-category__excerpt">' +
      ...
      searchResults.innerHTML =
        '<div class="posts-in-category__no-posts">Hakua vastaavia sisältöjä ei löytnyt.</div>';
    }
  }
